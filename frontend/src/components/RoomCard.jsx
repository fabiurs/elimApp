...
export default function RoomCard({ room, onSelect }) {
  return (
    <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] p-6 flex flex-col items-center transition-transform hover:scale-105 cursor-pointer border-2 border-transparent hover:border-indigo-400 group">
      ...
      <div className="flex flex-wrap gap-2 mb-2 justify-center">
        {room.amenities.map((a, i) => (
          <span key={i} className="bg-indigo-50 text-indigo-700 px-2 py-1 rounded-lg text-xs font-medium shadow-sm">{a}</span>
        ))}
      </div>
